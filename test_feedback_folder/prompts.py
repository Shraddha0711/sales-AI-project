...
def feedback_prompt(transcript):
    return f"""scenario_and_transcription : {transcript}
    Based on the given scenario and transcript, provide feedback on the customer service agent’s performance. The feedback should be presented in two parts:

The first part should be brief and concise, reflecting areas where the agent could improve. The second part should be more detailed, covering what the agent did, what the right approach or concept would have been, and how the agent could have applied the concept for better customer support.

Break the feedback into clear sections without labels, and avoid using titles for the concepts. Provide at least four tips for improvement and two things the agent did well, ensuring the advice is actionable and focused on both customer support principles and practical application.
